...
class Uploader {
	...
	private function generateFileName($conf, $file, $skip_replace_check = false){
        ...
        $file['name'] = $file['name'].'_'.substr(md5(rand()),0,7);
        ...
        $extension = !empty(strtolower($file['extension'])) ? "." . strtolower($file['extension']) : "";
        $string = "";
		...
		switch($type){
			...
			default:
                ...
                if(strpos($string, "{{extension}}") !== false){
                    ...
                    $string = str_replace("{{extension}}", strtolower($file['extension']), $string);
                }
                if(strpos($string, "{{.extension}}") !== false){
                    $is_extension_used = true;
